...
class TripsCreatorCiAbidjan(TripsCreator):
    ...
    def _service_id_from_transport_hour(self, a_transport_hour):
        service_days = [
            day_name
            for day_name in self._DAYS_OF_WEEK
        ...
        if collections.Counter(service_days) == collections.Counter(
            self._DAYS_OF_WEEK[:5]
        ):
            return date_range("monday", "friday")
        if collections.Counter(service_days) == collections.Counter(
            self._DAYS_OF_WEEK[:6]
        ):
            return date_range("monday", "saturday")
        if collections.Counter(service_days) == collections.Counter(
            self._DAYS_OF_WEEK[-2:]
        ):
            return date_range("saturday", "sunday")

        return ",".join(
            [self._DAY_ABBREVIATIONS[day_name] for day_name in service_days]
        )
            ...
            if "network" in line.tags and line.tags["network"]:
                try:
                    ...
                    if not agency.Validate():
                        logging.error(
                            "Agency data not valid for %s in line", line.tags["network"]
                        )
                ...
                for service_id, itinerary_hours in itinerary_hours_dict.items():
                    ...
                    if a_route.fr and a_route.to:
                        ...
                        if line_gtfs.route_short_name:
                            ...
                            line_gtfs.route_long_name = "{} ↔ {}".format(
                                a_route.fr, a_route.to
                            )

                    for itinerary_hour in itinerary_hours:
                        trip_gtfs.AddFrequency(
                            itinerary_hour["start_time"],
                    ...
                    for (
                        ...
                        stop_time,
                        is_timepoint,
                    ) in trip_gtfs.GetTimeInterpolatedStops():
                        if not is_timepoint:
